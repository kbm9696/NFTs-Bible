...
@st.cache_data
def fetch_analytics_data(blockchain, contract_address,time_range):
    """
    Fetch analytics data from the API with caching.
    """
    url = f'https://api.unleashnfts.com/api/v2/nft/collection/analytics?blockchain={blockchain}&contract_address={contract_address}&offset=0&limit=30&sort_by=sales&time_range={time_range}&sort_order=desc'
...
def collections_():
    ...
    contract = st.text_input("contract", "0x1e52f7a450b08b1b249a4f4f54518fc5278c2285")
    ...
    # Fetch data from API
    analytics_15m = fetch_analytics_data(option, contract, "15m")
    analytics_30m = fetch_analytics_data(option, contract, "30m")
    analytics_24h = fetch_analytics_data(option, contract, "24h")
